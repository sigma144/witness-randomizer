...
void Randomizer::AdjustSpeed() {
	...
	memory->WritePanelData<float>(0x17E74, OPEN_RATE, { 0.03f }); // Swamp Flood gate (inner), ´2.7x //Keeping these slower for now to help with EP's
	memory->WritePanelData<float>(0x1802C, OPEN_RATE, { 0.03f }); // Swamp Flood gate (outer), 2.7x
	...
	memory->WritePanelData<float>(0x17CC1, OPEN_RATE, { 0.8f }); // Quarry Elevator, 4x
