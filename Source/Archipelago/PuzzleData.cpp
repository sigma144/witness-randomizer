...
void PuzzleData::Read(std::shared_ptr<Memory> _memory) {
	...
	else if (id == 0x00557 || id == 0x005F1 || id == 0x00620 || id == 0x009F5 || id == 0x0146C || id == 0x3C12D || id == 0x03686 || id == 0x014E9) { // quarry erazor + stones (stones are incorrectly detected as b/w)
		hasStones = false;
		hasColoredStones = true;
	} 
